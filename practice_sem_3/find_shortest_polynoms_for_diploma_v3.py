...
from generalized_boolean_polynoms.practice_sem_3.utils import load_poly_id2min_poly_id_from_directory, int2binary_string
...
def main():
    ...
    output_dir = f"res_new/res_n_{num_vars}_new"
    if not os.path.exists(output_dir) and output_dir != '':
        os.makedirs(output_dir)

    input_p_id2min_p_id_dir = f"D:/AnRoot/University/VMK/сем_4_диплом/res_n_{num_vars}_new"
    ...
    unique_min_poly_ids_to_find = set(poly_id2min_poly_id.values())
    ...
    update_counter = 0
    batch_size = 10 ** 6
    permuted_new_poly_func_vector = np.zeros(shape=2 ** num_vars, dtype=np.int)
    ...
    while len(found_poly_function_ids_set) != expected_num_functions:
        ...
        for new_monom_positive_mask in allowed_positive_masks:
            ...
            for monom in monoms_list:
                ...
                new_poly_function_id = (two_degrees * new_poly_func_vector).sum()
                ...
                # Если такую функцию уже видел, перехожу к следующему моному
                # TODO
                # if new_poly_min_f_id in found_poly_function_ids_set:
                #     continue
                monom_str = monom.monom_str
                # TODO: Вот я получил айди новой функции. Что дальше? Он может быть не минимальным.
                # TODO: Если он минимальный, то я его просто добавляю в очередь на обход
                # TODO: Если он не минимальный, мне надо перестановками найти минимальный
                # TODO: Вот я нашёл перестановочный минимальный. Мне надо:
                # TODO: 1. новый список мономов. Это самое сложное
                # TODO: 2. Новый вектор значений функции. Это легко -- надо просто взять новый вектор
                if new_poly_function_id in unique_min_poly_ids_to_find:
                    ...
                    if len(writing_batch) > batch_size:
                        ...
                        writing_batch.clear()
                    # TODO: Добавить в батч
                else:
                    # TODO: Где-то всякие ассерты или ифы на то, что функция ещё не найдена
                    ...
                    for perm in perms:
                        ...
                        permuted_new_poly_function_id = (two_degrees * permuted_new_poly_func_vector).sum()
